...
echo -e "${CYAN}======================================${NC}"
echo -e "${CYAN}  NativeBeam Build Script${NC}"
echo -e "${CYAN}======================================${NC}"
echo ""
...
if ! dotnet clean NativeBeam.slnx -c "$CONFIGURATION"; then
    echo ""
...
if ! dotnet restore NativeBeam.slnx; then
    echo ""
...
if ! dotnet build NativeBeam.slnx -c "$CONFIGURATION" --no-restore; then
    echo ""
...
if [ "$SKIP_TESTS" = false ]; then
    ...
    if ! dotnet test NativeBeam.slnx -c "$CONFIGURATION" --no-build --verbosity normal; then
        echo ""
