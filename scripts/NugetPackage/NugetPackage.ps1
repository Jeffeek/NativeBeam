...
$Projects = @{
    "Pdf" = "src\NativeBeam.Pdf\NativeBeam.Pdf.csproj"
}
...
foreach ($nupkgFile in Get-ChildItem $OutputDir -Filter *.nupkg) {
    ...
    # Check README.md (logo is optional for NativeBeam)
    $readme = Get-ChildItem $extractPath -Recurse -Filter "README.md" -ErrorAction SilentlyContinue
    if ($readme) {
...
Write-Host "  - Test locally:            dotnet add package NativeBeam.Pdf --source .\test-packages" -ForegroundColor Gray
Write-Host ""
