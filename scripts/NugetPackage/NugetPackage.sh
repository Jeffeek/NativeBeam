...
declare -A PROJECTS
PROJECTS["Pdf"]="src/NativeBeam.Pdf/NativeBeam.Pdf.csproj"
...
for NUPKG in "$OUTPUT_DIR"/*.nupkg; do
    if [ -f "$NUPKG" ]; then
        ...
        echo -e "${CYAN}  $(basename "$NUPKG") (${SIZE} KB)${NC}"
    fi
done
...
for NUPKG in "$OUTPUT_DIR"/*.nupkg; do
    if [ -f "$NUPKG" ]; then
        ...
        find "$EXTRACT_PATH" -type f \( -name "*.dll" -o -name "*.xml" -o -name "README.md" -o -name "*.png" \) | while read -r FILE; do
            ...
            echo -e "${GRAY}  $(basename "$FILE") (${FILE_SIZE} KB)${NC}"
        done
    fi
done
...
echo -e "${GRAY}  - Test locally:            dotnet add package NativeBeam.Pdf --source ./test-packages${NC}"
echo ""
